...
const formatDuration = (duration) => {
  ...
  if (typeof duration === "string") {
    ...
    if (hoursMatch && minsMatch) return `${hoursMatch[1]}h ${minsMatch[1]}m`;

    if (hoursMatch) return `${hoursMatch[1]}h 0m`;

    if (minsMatch) {
      const totalMinutes = parseInt(minsMatch[1], 10);
...
* Convert a time string and optional am/pm into a 24-hour HH:MM string.

* Handles inputs like "4:00", "04:00", "16:00", "4:00 PM", etc.

* If ampm param is provided, we only apply it when the hour is <= 12 (to avoid double-applying when time already 24h).
...
function to24Hour(timeStr = "00:00", ampm = "") {
  ...
  if (!explicitMatch) return "00:00";

  let h = Number(explicitMatch[1]);

  let m = explicitMatch[2] ? explicitMatch[2] : "00";

  const inTimeAmp = explicitMatch[3] ? explicitMatch[3].toUpperCase() : "";

  const paramAmp = (ampm || "").toUpperCase();

  // Decide which AM/PM to use: prefer the one included in the time string itself.

  // Only use the param ampm if the parsed hour looks like a 12-hour value (<=12)

  const useAmp = inTimeAmp || (paramAmp && h <= 12 ? paramAmp : "");
  ...
  // If hour ended up > 23 due to bad input, clamp to 23
...
* Accept variety of slot shapes and produce an ISO datetime string (with +05:30 offset)
...
function slotToISO(slot) {
  ...
  if (typeof slot === "string") {
    // If string looks like full ISO, return as-is; otherwise attempt to parse as date

    if (/^\d{4}-\d{2}-\d{2}T/.test(slot)) return slot;

    // if string is date only

    if (/^\d{4}-\d{2}-\d{2}$/.test(slot)) return `${slot}T00:00:00+05:30`;
  ...
  if (typeof slot === "object") {
    // preferred: date + time (or iso)

    if (slot.datetime) {
      return slot.datetime;
    }

    // handle explicit iso field
    ...
    if (slot.date) {
      ...
      // Accept slot.time possibly containing AM/PM and/or numeric hours

      const timeStr = slot.time || slot.timeStr || "";

      const ampm = slot.ampm || slot.amp || slot.AMPM || "";

      const hhmm = to24Hour(timeStr || "00:00", ampm);

      // build a timezone-aware ISO-like string: YYYY-MM-DDTHH:MM:00+05:30
...
/** Normalize movie data for use in UI (normalize URLs, previews, etc.) */

function normalizeMovieForClient(it = {}) {
  if (!it) return null;

  const item = { ...it };

  // pick poster/thumbnail priority

  item.poster =
    item.poster ||
    (item.latestTrailer && item.latestTrailer.thumbnail) ||
    item.thumbnail ||
    item.posterUrl ||
    null;

  // normalize poster to absolute URL if needed
  ...
  // ensure slots is array

  item.slots = Array.isArray(item.slots)
    ? item.slots
    : item.slots
    ? [item.slots]
    : [];
...
export default function MovieDetailPage() {
  ...
  // navigation may pass full movie object: use as fast fallback

  const initialFromStateMovie = location?.state?.movie || null;

  const [movie, setMovie] = useState(
    initialFromStateMovie
      ? normalizeMovieForClient(initialFromStateMovie)
      : null
  );

  const [loading, setLoading] = useState(!initialFromStateMovie);
    ...
    (async function fetchMovie() {
      ...
      try {
        ...
        if (!data || data.success === false) {
          ...
          // do not clobber existing local state if we have it

          if (!initialFromStateMovie) setMovie(null);
        } else {
          const itemRaw =
            data.item || data.data || (data.success ? data : null);

          const item = normalizeMovieForClient(itemRaw);
        ...
        // If we already have movie from navigation state, keep it and notify user

        if (!initialFromStateMovie) {
          toast.error("Failed to fetch movie from server");

          setMovie(null);
        } else {
          toast.info("Using cached movie data (failed to refresh).");
        }
      } finally {
  ...
  }, [movieIdParam]); // keep only id to avoid refetch loops
